...
def fit_loop(model, fit_function, fit_inputs,
	...
	print("do_validation = " + str(do_validation))
	...
	print(feed)
	"""
	with open("modelShow.txt", "w") as f:
		f.write("Model before epochs:\n")
		layerNumber = 0
		for ll in model.layers:
			f.write("layer number " + str(layerNumber) + " out of " + str(len(model.layers)) + "\n")
			layerNumber += 1
			weights = ll.get_weights()
			f.write("There are " + str(len(weights)) + " weights\n")
			for ww in weights:
				f.write("There are " + str(len(ww)) + " values\n")
				if len(ww) < 1000:
					c = 0
					for www in ww:
						f.write("{" + str(www) + "}")
	...
	for epoch in range(initial_epoch, epochs):
		...
		#	print(wei)
		"""
		with open("modelShow.txt", "a") as f:
			f.write("Model after epoch " + str(epoch) + " out of " + str(epochs) + " epochs\n")
			layerNumber = 0
			for ll in model.layers:
				f.write("layer number " + str(layerNumber) + " out of " + str(len(model.layers)) + "\n")
				layerNumber += 1
...
def predict_loop(model, f, ins,
	...
	num_samples = check_num_samples(ins,
									...
									steps_name='steps')
	"""
	print("Within predict_loop...")
	print("batch_size = " + str(batch_size))
	print("num_samples = " + str(num_samples))
	print("length of ins = " + str(len(ins)))
	print("ins:")
	print(ins)
	#exit()
	"""
	...
	#print("Length of indices_for_conversion_to_dense = " + str(len(indices_for_conversion_to_dense)))
	#exit()
	...
	if steps is not None:
		...
		#print("length of outs = " + str(len(outs[0])))
		#print("Outs:")
		#print(outs)
		#exit()
