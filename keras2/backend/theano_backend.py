...
class Function(object):
	...
	def __init__(self, inputs, outputs, updates=[], name=None, **kwargs):
		...
		print("***There are " + str(len(updates)) + " updates.")
		print("***updates:")
		print(updates)
		...
		print("*** there are " + str(len(self._metrics)) + " metrics. ***")
		print(self._metrics)
		if len(self._metrics) > 0:
			print("***The first metric:")
			print(self._metrics[0])
	...
	def __call__(self, inputs):
		print("***Function __call__ is called***")
		print("There are " + str(len(inputs)) + " inputs")
		if False and len(inputs) > 0:
			print("The first input is " + str(inputs[0]))
		assert isinstance(inputs, (list, tuple))
		outputs = self.function(*inputs)
		if self._metrics:
			metrics = self._metrics_function()
			print("There are " + str(len(metrics)) + " metrics")
			if len(metrics) > 0:
				print("The first metric:")
				print(metrics[0])
		i = 0
		...
		for x in self.outputs:
			if hasattr(x, '_is_metric'):
				...
				j += 1
			i += 1
		print("There are " + str(len(outputs)) + " outputs")
		print("Outputs:")
		for oo in outputs:
			print(oo)
		return outputs
