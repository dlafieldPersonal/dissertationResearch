...
void *glove_thread(void *vid) {
    ...
    for(a = 0; a < lines_per_thread[id]; a++) {
        ...
        if(cr.word1 == 0 || cr.word2 == 0) continue;   // Doug Lafield fixed a bug
        if(feof(fin)) break;
        ...
        crVal = cr.val;
        if(crVal == 0.0) crVal = 0.0001;   // Doug Lafield fixed a bug
        diff += W[vector_size + l1] + W[vector_size + l2] - log(crVal); // add separate bias for each word
        fdiff = (crVal > x_max) ? diff : pow(crVal / x_max, alpha) * diff; // multiply weighting function (f) with diff
